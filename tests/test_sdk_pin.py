"""Pin SDK v `pyproject.toml` musí souhlasit s `.sdk-ref`.

Obě místa drží tentýž commit SDK: závislost v `pyproject.toml` (odkud
instaluje běžný uživatel) a `.sdk-ref` (podle kterého se materializuje SDK při
buildu). Kdyby se rozešly, uživatel by dostal jinou verzi SDK než tu, proti
které se konektor testuje.

Na rozdíl od ostatních konektorů tu zatím není `release/vendor/`, takže
vendorovaný archiv se nekontroluje — až přibude, patří sem třetí test.
"""
